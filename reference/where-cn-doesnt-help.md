...
- **Eval and regression: cloud native runs it, it does not score it.** Whether the new model got worse at your top intent is an eval question, and cloud native has no opinion on the score. But running the eval as a Job, gating a rollout on its result, shifting traffic with a canary, and rolling back automatically are all cloud native. See [Pain S.04](../pains/S04-quality-gates.md). The verdict is yours; the gate is the platform's.
- **Prompt quality.** Cloud native can version your prompts and ship them safely ([Pain G.01](../pains/G01-prompt-version.md)). It cannot tell you whether they are any good.
