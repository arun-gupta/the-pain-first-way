...
- **Per-tenant and per-agent scoping**: different agents get different allowlists, so a compromised one can't reach another's targets, which connects to [Pain R.02](R02-tenant-isolation.md).
