# Pain 29: A prompt-injected agent can call any endpoint and walk my data out
...
- **Per-tenant and per-agent scoping**: different agents get different allowlists, so a compromised one can't reach another's targets, which connects to [Pain 25](25-tenant-isolation.md).

This bounds the blast radius that [Pain 27](27-agent-sandbox.md) contains locally. Together they limit what a misbehaving agent can do. Cloud native can't prevent the injection itself, that's the AI layer, only what it can reach.
...
[← Pain 28: Tool and MCP fleet](28-tool-fleet.md) · [Landscape](../README.md) · [Pain 30: Runaway-loop governance →](30-runaway-agents.md)
