# Pain S.02: My server image is coupled to its config and secrets
...
A working demonstration lives in [`examples/S02-image-coupling/`](../examples/S02-image-coupling/). The before case runs without Docker or Kubernetes and shows the hardcoded source URL and credentials printed on every startup. The after case uses an init container, a ConfigMap, and a Secret to decouple them from the image; the walkthrough includes steps to simulate a key rotation and a source change without rebuilding. Runnable on a Mac with a local Kind cluster and no GPU required.

---

[← Pain S.01: Cold start](S01-cold-start.md) · [Landscape](../README.md) · [Pain S.03: Can't roll back →](S03-cant-roll-back.md)
