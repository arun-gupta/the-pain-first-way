# Pain 14: I run training jobs on SLURM and Kubernetes feels like a rewrite
...
- **MPI Operator, PyTorchJob, `torchrun`**: `srun --ntasks` multi-node runs map to a training operator that gang-schedules ranks and wires up the rendezvous. See [Pain 5](05-multi-node-training.md).
...
**What you give up**: `sbatch` muscle memory and a single controller that owns the queue. You now think in declared objects reconciled by control loops, not scripts submitted to a daemon. For teams that want the bridge instead of the rewrite, see [Pain 15](15-slurm-bridge.md).

---

[← Pain 13: Data residency](13-data-residency.md) · [Landscape](../README.md) · [Pain 15: SLURM bridge →](15-slurm-bridge.md)
