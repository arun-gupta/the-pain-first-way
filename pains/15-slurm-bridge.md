# Pain 15: My team knows SLURM and won't relearn everything to run one job
...
The full rewrite ([Pain 14](14-slurm-migration.md)) is correct and also slow, because it asks every user to change at once. There is an incremental path: keep the SLURM interface, swap the engine underneath. Slinky runs a SLURM control plane as a Kubernetes operator. Users still type `sbatch`, `squeue`, and `scancel`. The jobs run as Kubernetes pods. The infrastructure migrates without retraining the people on top of it.

**Rewrite path vs bridge path:**

```mermaid
flowchart TB
  subgraph Rewrite["Full rewrite (Pain 14)"]
    direction LR
...
- **What changes underneath**: the backing scheduler is Kubernetes. GPUs are managed by device plugins or [DRA](04-whole-gpus-only.md). Capacity grows with HPA and Karpenter instead of static node pools.
