# Pain C.02: I can't get a GPU when I need one
...
A working demonstration lives in [`examples/C02-queueing/`](../examples/C02-queueing/). Three experiment jobs compete for two slots; a fourth production job preempts one and is admitted immediately. Runnable on a Mac with a local Kind cluster and no GPU required. The only change to a Job manifest is two lines.

---

[← Pain C.01: GPU job crashed](C01-gpu-job-crashed.md) · [Landscape](../README.md) · [Pain C.03: Can't express GPU config →](C03-whole-gpus-only.md)
