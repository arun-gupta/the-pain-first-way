# Pain O.05: A GPU degrades mid-job and Kubernetes doesn't notice
...
This is distinct from [Pain C.01](../compute/C01-gpu-job-crashed.md), recovering a crashed job by checkpoint and restart. This pain is the cluster noticing the hardware is bad in the first place, so the job isn't rescheduled straight back onto the same failing device. It also surfaces faults that inference latency metrics ([Pain O.02](O02-latency-spiked.md)) never explain.
...
[← Pain O.04: Costs out of control](O04-costs-out-of-control.md) · [Landscape](../../README.md) · [Pain G.01: Prompt version in prod →](../governance/G01-prompt-version.md)
